...
export function ObjectDetailsPane({ connection, database, object, details, onPreview, onSend }: ObjectDetailsPaneProps) {
  ...
  if (object === null) {
    ...
    const product = database?.product ?? capabilities.label
    const version = database?.version ?? connection.versionHint ?? capabilities.versionRange
    return <section className={css.detailsPane} data-sql-database-details>
      <header className={css.detailsHeader}><VscTable size={18} /><div><h2>{database?.name ?? connection.name}</h2><span>{product} · {version}</span></div></header>
      <dl className={css.detailsGrid}>
        <div><dt>{t('details.connection')}</dt><dd>{connection.name}</dd></div>
        <div><dt>{t('details.product')}</dt><dd>{product}</dd></div>
        <div><dt>{t('details.version')}</dt><dd>{version}</dd></div>
        {connection.kind === 'sqlite' ? <div><dt>{t('field.file')}</dt><dd title={connection.file}>{connection.file}</dd></div> : <>
          <div><dt>{t('field.host')}</dt><dd>{connection.host}:{connection.port}</dd></div>
          <div><dt>{t('field.user')}</dt><dd>{connection.user || '-'}</dd></div>
        </>}
        {database?.charset != null && <div><dt>{t('details.charset')}</dt><dd>{database.charset}</dd></div>}
        {database?.collation != null && <div><dt>{t('details.collation')}</dt><dd>{database.collation}</dd></div>}
        {connection.description != null && connection.description !== '' && <div><dt>{t('field.description')}</dt><dd title={connection.description}>{connection.description}</dd></div>}
        {connection.tags !== undefined && connection.tags.length > 0 && <div><dt>{t('field.tags')}</dt><dd>{connection.tags.join(', ')}</dd></div>}
  ...
  return <section className={css.detailsPane} data-sql-object-details>
    <header className={css.detailsHeader}>{value.kind === 'table' ? <VscTable size={18} /> : <VscEye size={18} />}<div><h2>{value.name}</h2><span>{details?.qualifiedName ?? value.name} · {value.kind}</span></div><span className={css.toolbarSpacer} /><button className={css.commandButton} title={t('menu.preview')} onClick={() => { onPreview(value) }}><VscEye />{t('menu.preview')}</button><button className={css.commandButton} title={t('menu.add')} onClick={() => { onSend(value) }}><VscComment />{t('menu.add')}</button></header>
    <dl className={css.detailsGrid}>
    ...
    {details !== null && details.constraints.length > 0 && <>
      <div className={css.detailsSectionTitle}>{t('details.constraints')}</div>
      <div className={css.detailsTableScroll}><table className={css.resultTable}><thead><tr><th>{t('details.name')}</th><th>{t('details.type')}</th><th>{t('details.columns')}</th><th>{t('details.definition')}</th></tr></thead><tbody>{details.constraints.map(constraint => <tr key={constraint.name}><td>{constraint.name}</td><td>{t(('constraint.' + constraint.kind) as Parameters<typeof t>[0])}</td><td>{constraint.columns.join(', ') || '-'}</td><td title={constraint.definition ?? undefined}>{constraint.definition ?? '-'}</td></tr>)}</tbody></table></div>
    </>}
